...
fn write_file_contents<'data, P: Platform<'data, File = crate::elf::File<'data>>>(
    ...
    let global_sym_index_map = if layout.args().should_output_reloc {
...
fn write_program_headers(
    ...
    if layout.args().should_output_reloc {
        return Ok(());
    }
...
fn build_global_sym_index_map(layout: &ElfLayout<'_>) -> Result<Vec<Option<u32>>> {
...
fn write_rela_sections<'data>(
    object: &ObjectLayout<'data, elf::File<'data>>,
    buffers: &mut OutputSectionPartMap<&mut [u8]>,
    layout: &ElfLayout<'data>,
    global_sym_index_map: &[Option<u32>],
        ...
        .zip(layout.per_symbol_flags.raw_range(object.symbol_id_range))
    {
        if sym.st_type() == object::elf::STT_SECTION {
            if let Ok(Some(input_section_index)) = object.object.symbol_section(sym, sym_index) {
...
fn write_section_headers(out: &mut [u8], layout: &ElfLayout) -> Result {
    ...
    while let Some(event) = order.next() {
        ...
        if layout.args().should_output_reloc && section_type == sht::RELA && section_id.is_custom()
        {
            ...
            if let Some(section_name) = output_sections.name(section_id) {
                if let Some(target_name) = section_name.0.strip_prefix(b".rela") {
                    if let Some(target_id) = output_sections
                        .custom_name_to_id(crate::output_section_id::SectionName(target_name))
                    {
                        if let Some(target_idx) = output_sections.output_index_of_section(target_id)
                        {
                            info_value = target_idx as u32;
                        }
                    }
                }
            }
        ...
        entry.sh_addr.set(
            e,
            if layout.symbol_db.output_kind.is_partial_object() {
                0
            } else {
